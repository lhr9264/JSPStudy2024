...
public class BoardDAO extends JDBConnect {
	...
	// 게시물의 개수를 카운트하여 'int'형으로 반환한다.
	public int selectCount(Map<String, Object> map) {
		int totalCount = 0;
		// 게시물의 수를 얻어오기 위한 쿼리문 작성
		String query = "SELECT COUNT(*) FROM board";
		// 검색어가 있는경우 'where'절을 추가한다.
		if (map.get("searchWord") != null) {
			query += " WHERE " + map.get("searchField") + " "
					+ " LIKE '%" + map.get("searchWord") + "%' ";
		} try {
			// 정적쿼리문 실행 실행시 'Statement'인스턴스 생성
			stmt = con.createStatement();
			// 쿼리 실행 및 결과 반환
			rs = stmt.executeQuery(query);
			// ResultSet객체에서 결과값을 읽음
			rs.next();
			// 첫 번째 컬럼의 값을 얻어온 후 변수에 저장
	...
	public List<BoardDTO> selectList(Map<String, Object> map) {
		/*
		 * List 계열의 컬렉션을 생성
		 * 이때 타입 매개변수는 'board'테이블을 대상으로 하므로 BoardDTO로 설정한다.
		 */
		...
		try {
			...
			while(rs.next()) {
				...
				// 'setter'를 이용해서 각 컬럼의 값을 멤버변수에 저장
	...
	public int insertWrite(BoardDTO dto) {
		...
		try {
			/*
			 * 인파라미터가 있는 동적쿼리문을 작성한 후 유저가 입력한 값으로 설정한다.
			 * 일련번호는 시퀀스로 자동부여하고, 조회수는 0으로 입력한다.
			 */
	...
	public BoardDTO selectView(String num) {
		...
		/*
		 * 회원의 이름까지 인출하기 위해 'member'테이블과 
		 * 내부조인을 설정하여 레코드를 가져온다.
		 */
