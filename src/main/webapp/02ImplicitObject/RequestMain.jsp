...
<title>내장 객체 - request</title>
	...
	2.post방식
		...
		-전송되는 용량에 제한이 없다.
		-파일 첨부시에는 반드시 post방식으로 전송해야한다.
	 -->
	    <h2>1. 클라이언트와 서버의 환경정보 읽기</h2>
	    <!-- GET 방식으로 요청 -->
	    <!-- 
	    <a>태그를 통해 페이지 이동시 파라미터를 전달하고 싶다면 파일명 뒤에 물음표를
	    붙이고 파라미터명=값 의 형태로 작성한다. 2개 이상의 파라미터는 &를 통해
	    구분하면 된다. 이 부분을 "쿼리스트링(Querystring)" 이라고 한다.
	     -->
	    ...
	    <br />
	    <!-- POST 방식으로 요청-->
	    <!-- <from>태그 하위의 <input>, <select>, <textarea>태그를
	    통해 값을 전송할때는 반드시 name속성이 명시되어야 한다. name 속성에
	    지정한 이름ㅇ르 통해 서버로 파라미터를 전송할 수 있다. --> 
